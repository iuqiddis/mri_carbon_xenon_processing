function [fnorm8,RE,IM, deblurring] = csireconabs_sms(path,lb,deblurring, scaling)
% [fnorm8,RE,IM] = csirecon(path,lb,deblurring, scaling)
% 
% Changelog
...
% 
% 11/9/18 -SMS
% Adding a section for automatic deblurring. 'deblurring' can be set to a
% numerical value as before, or set to 'auto' for an exponential fit to
% calculate the deblurring constant. The 'scaling' factor is another
% parameter to tune the applied deblurring after a fit. 0.4 seems to be a
% sane default
...
% for fitting the deblurring constant
if strcmp(deblurring,'auto')
    if nargin == 3
        scaling = 0.4;
        display('No scaling factor for deblurring provided. Default will be 0.4')
    end
    
    [pt, nt] = size(RE1);
    m=1;
    for n=1:11:nt % comes back to k=0 every 11th acquisition
        re0(:,m) = RE1(:,n);
        np(m) = pt*(n-1);
        m = m+1;
    end
    ...
    mre0 = max(re0);
    coef = fit(np', mre0', 'exp1'); 
    deblurring = scaling*-coef.b';
    fprintf('\n''deblurring'' value used = %f\n', deblurring)
    
    figure(119)
    tr = readprocpar(path,'tr'); tr = tr(2);
    tr0 = np*tr/128; % changing points to seconds
    plot(tr0, mre0,'o-',tr0,coef.a.*exp(coef.b*np))
    legend('raw k_0', 'fit')
    title('Fit of k_0 signals to an exponential')
    xlabel('seconds')
end
