...
% update(1) on 9/7/2016 to include transplant data
%
% update(2) on 9/13/2016 to use steve's fits for ratio measurements
% and saves individual ones. 
%
% update(3) on 9/14/2016 now has the option to do the voxel selection
% based on an MRSI overlay or a carbon map.
%
% update(4) on 10/07/2016, dataset number is now added to the name of
% side voxel files.
%
% update(5) on 10/19/2016, allows the user to select five regions in
...
% well.
%
% Update (6) on 11/3/2016 modified for selection of the sides.
% This code uses the aboslute value of the peak
%
% Update (7) on 02/23/2017 easier to deal with file names
%
% Update (8) on 03/12/2017 renamed the file, now it deals with two
% sides, useful for quantifying the abnormal side and its contralateral
% side
%
% Update (9) on 06/21/2017 saves individual voxels as well
%
% update(10) on 01/21/2018
...
for mm = 1:2
    ...
    lac = lac_image(:).*mask_sides(:);
    pyr = pyr_image(:).*mask_sides(:); pyr = pyr(lac>1e-10)';
    bic = bic_image(:).*mask_sides(:); bic = bic(lac>1e-10)';
    ala = ala_image(:).*mask_sides(:); ala = ala(lac>1e-10)';
    lac = lac(lac>1e-10)';
    num_vox = round(length(lac(lac>1e-10)'));
